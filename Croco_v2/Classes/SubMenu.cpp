...
SubMenu::SubMenu(void)
{
	...
	//add leather
	CCSprite* lth = CCSprite::create(SubMenu_lead_choise_groups);
    addChild(lth,0);
    lth->setAnchorPoint(ccp(0.5,1));
	lth->setPosition(ccp(x,y*2));
    
	//menu icons
	CCMenuItem* itm_3gamers = CCMenuItemImage::create(SubMenu_3gamers,SubMenu_3gamers,this,menu_selector(SubMenu::itm_3gamersCallback));
	CCMenuItem* itm_2groups = CCMenuItemImage::create(SubMenu_2groups,SubMenu_2groups,this,menu_selector(SubMenu::itm_2groupsCallback));
	CCMenuItem* itm_3groups = CCMenuItemImage::create(SubMenu_3groups,SubMenu_3groups,this,menu_selector(SubMenu::itm_3groupsCallback));
	CCMenuItem* itm_4groups = CCMenuItemImage::create(SubMenu_4groups,SubMenu_4groups,this,menu_selector(SubMenu::itm_4groupsCallback));
	CCMenuItem* itm_back = CCMenuItemImage::create(GroupCustomization_back,GroupCustomization_back_selected,this,menu_selector(SubMenu::itm_backCallback));
	CCMenu* menu = CCMenu::create(itm_3gamers,itm_2groups,itm_3groups,itm_4groups,itm_back,NULL);
	addChild(menu);
...
void SubMenu::GoGroupCustomization(int GameMode)
{
	...
	CCScene* s = CCTransitionFade::create(1.0f, pScene);
	pLayer->release();
